  ...
  const handleTrigger = async () => {
    ...
    try {
      ...
      const ref = mutationRef(dataconnect, 'LogEmergencyEvent', {
        ...
        details: `STAFF_REQUISITION: ${selectedIds.join(', ')}`,
        lat: coords.lat,
        lng: coords.lng
      });
      await executeMutation(ref);
      
      console.log('Staff request logged successfully');
    } catch (err) {
      console.error('Failed to log staff request:', err);
      // Fallback
      try {
        const fallbackRef = mutationRef(dataconnect, 'LogEmergencyEvent', {
          type: 'STAFF',
          priority: 'PRIORITY_SERVICE',
          details: `STAFF_REQUISITION: ${selectedIds.join(', ')} (NO_LOCATION)`,
          lat: null,
          lng: null
        });
  ...
  const categories = [
    {
      id: 'usher',
      title: 'Request Usher',
      desc: 'Guidance required for seating or venue navigation. Nearby staff will be notified.',
      icon: 'person_search',
      priority: 'STANDARD',
      color: 'primary',
    ...
    },
    {
      id: 'technical',
      title: 'Technical Fix',
      desc: 'Seat malfunction or technical issue. Specialized maintenance dispatch.',
    ...
    },
    {
      id: 'disability',
      title: 'ADA Support',
      desc: 'Assistance requested for accessibility requirements or mobility support.',
      icon: 'accessible',
      priority: 'URGENT',
      color: 'tertiary',
      colSpan: 'md:col-span-1'
    },
    {
      id: 'other',
      title: 'General Help',
      desc: 'Any other staff-related requirements. A team member will approach your sector.',
    ...
    <div className="bg-surface font-body text-on-surface selection:bg-primary-container min-h-screen w-full flex flex-col relative overflow-x-hidden">
      ...
      <header className="fixed top-0 w-full z-50 bg-[#0B1326]/60 backdrop-blur-md flex justify-between items-center h-16 px-6 shadow-[0_1px_0_0_rgba(219,226,253,0.05)]">
        <div className="flex items-center gap-3">
          <span className={`material-symbols-outlined ${isTriggered ? 'text-tertiary animate-pulse' : 'text-primary'}`} data-icon="support_agent">support_agent</span>
          <span className="text-sm font-bold tracking-[0.2em] text-[#DBE2FD] font-headline uppercase">
            {isTriggered ? 'Staff Alerted' : 'Concierge Relay'}
          </span>
        </div>
      ...
      <main className="flex-1 flex flex-col pt-20 pb-24 px-6 relative overflow-y-auto scroll-smooth text-on-surface">
        ...
        <section className="grid grid-cols-2 gap-4 mb-6">
          <div className="glass-card p-4 rounded-xl border border-white/5">
            <p className="font-label text-[10px] uppercase tracking-widest text-on-surface-variant mb-1">TEAM RELAY</p>
            <p className={`font-headline text-2xl font-bold ${isTriggered ? 'text-secondary animate-pulse' : 'text-primary'}`}>
          ...
          <div className="absolute top-0 right-0 p-4">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-[10px] font-label uppercase tracking-widest ${isTriggered ? 'text-secondary' : 'text-primary'}`}>
                {isTriggered ? 'Relay Linked' : 'Signal Open'}
              </span>
            </div>
          </div>
          <label className="font-label text-[10px] uppercase tracking-[0.3em] text-on-surface-variant block mb-1">Your location</label>
        ...
        >
          ...
          <div className="relative flex items-center justify-center gap-4 z-10">
            <span className="material-symbols-outlined text-white text-3xl" data-icon={isTriggered ? "how_to_reg" : "send"} style={{ fontVariationSettings: "'FILL' 1" }}>{isTriggered ? "how_to_reg" : "send"}</span>
            <span className="font-headline font-bold text-xl uppercase tracking-[0.15em] text-white">
              {isTriggered ? "Request Sent" : "Call for Staff"}
            </span>
          </div>
