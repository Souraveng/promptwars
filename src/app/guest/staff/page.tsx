...
export default function GuestStaffPage() {
  ...
  const categories = [
    {
      id: 'directional',
      title: 'Directional Help',
      desc: 'Assistance finding specific gates, amenities, or sections within the venue.',
      icon: 'directions',
      priority: 'STANDARD',
      color: 'secondary',
    ...
    {
      id: 'ticket',
      title: 'Ticket / Seat Issue',
      desc: 'Problems with seat access, double bookings, or digital ticket validation.',
    ...
    {
      id: 'concession',
      title: 'Concession Help',
      desc: 'Issues with ordered items, payment failures, or dietary requirement queries.',
      icon: 'restaurant',
      priority: 'STANDARD',
      color: 'secondary',
      colSpan: 'md:col-span-1'
    },
    {
      ...
      desc: 'Any other questions or feedback regarding the venue experience.',
    ...
    <div className="bg-surface font-body text-on-surface selection:bg-primary-container min-h-screen w-full flex flex-col relative overflow-x-hidden">
      ...
      <header className="fixed top-0 w-full z-50 bg-[#0B1326]/60 backdrop-blur-md flex justify-between items-center h-16 px-6 shadow-[0_1px_0_0_rgba(219,226,253,0.05)]">
        <div className="flex items-center gap-3">
          <span className={`material-symbols-outlined ${isTriggered ? 'text-secondary animate-pulse' : 'text-primary'}`} data-icon="support_agent">support_agent</span>
          <span className="text-sm font-bold tracking-[0.2em] text-[#DBE2FD] font-headline uppercase">
            {isTriggered ? 'Contact Established' : 'Staff Assistance'}
          </span>
        </div>
      ...
      <main className="flex-1 flex flex-col pt-20 pb-24 px-6 relative overflow-y-auto scroll-smooth">
        ...
        <section className="grid grid-cols-2 gap-4 mb-6">
          <div className="glass-card p-4 rounded-xl border border-white/5">
            <p className="font-label text-[10px] uppercase tracking-widest text-on-surface-variant mb-1">AVAILABILITY</p>
            <p className={`font-headline text-2xl font-bold ${isTriggered ? 'text-secondary animate-pulse' : 'text-secondary'}`}>
              {isTriggered ? 'STATIONED' : 'ACTIVE'}
            </p>
          </div>
          <div className="glass-card p-4 rounded-xl border border-white/5 text-right">
            <p className="font-label text-[10px] uppercase tracking-widest text-on-surface-variant mb-1">STAFF PROXIMITY</p>
            <p className="font-headline text-2xl font-bold text-primary">{isTriggered ? 'ARRIVING' : 'CLOSE'}</p>
          </div>
        </section>
        ...
        <section className="glass-card p-6 rounded-2xl mb-6 relative overflow-hidden border border-white/5">
          <div className="absolute top-0 right-0 p-4">
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${isTriggered ? 'bg-secondary animate-ping shadow-[0_0_15px_#4edea3]' : 'bg-secondary animate-pulse shadow-[0_0_10px_#4edea3]'}`}></div>
              <span className={`text-[10px] font-label uppercase tracking-widest text-secondary`}>
                {isTriggered ? 'Beacon Active' : 'Location Ping'}
              </span>
            </div>
          </div>
        ...
        <button 
          onClick={handleTrigger}
          className={`w-full h-20 rounded-2xl relative overflow-hidden group mb-4 transition-all ${isTriggered ? 'opacity-100 shadow-[0_0_40px_rgba(16,185,129,0.3)]' : ''}`}
        >
          <div className={`absolute inset-0 bg-gradient-to-r ${isTriggered ? 'from-[#059669] to-[#047857]' : 'from-secondary to-[#059669]'} transition-transform duration-500 group-hover:scale-105`}></div>
          <div className={`absolute inset-0 ${isTriggered ? '' : 'shadow-[0_0_40px_rgba(16,185,129,0.3)]'}`}></div>
          <div className="relative flex items-center justify-center gap-4 z-10">
            <span className="material-symbols-outlined text-white text-3xl" data-icon={isTriggered ? "forum" : "person_search"} style={{ fontVariationSettings: "'FILL' 1" }}>{isTriggered ? "forum" : "person_search"}</span>
            <span className="font-headline font-bold text-xl uppercase tracking-[0.15em] text-white">
              {isTriggered ? "Staff Notified" : "Call Staff Member"}
            </span>
          </div>
