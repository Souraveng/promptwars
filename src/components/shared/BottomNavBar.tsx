      ...
      {links.map((link) => {
        ...
        const content = (
          <div className={`flex flex-col items-center justify-center transition-all ${!isLocked && 'active:scale-90'} duration-150 h-full w-full ${iconColor} ${isActive && !isLocked && "relative after:content-[''] after:absolute after:bottom-0 after:w-1 after:h-1 after:bg-secondary after:rounded-full after:shadow-[0_0_8px_var(--color-secondary)]"}`}>
            <span className="material-symbols-outlined mb-1" style={isActive && !isLocked ? { fontVariationSettings: "'FILL' 1" } : {}}>
        ...
        if (isLocked) {
          return <div key={link.href} className="flex-1 h-full cursor-not-allowed grayscale opacity-50">{content}</div>;
        }

        return (
          <Link key={link.href} href={link.href} className="flex-1 h-full">
            {content}
          </Link>
        );
      })}
